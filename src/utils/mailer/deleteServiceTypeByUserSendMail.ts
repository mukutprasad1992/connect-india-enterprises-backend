...
import { notificationSentSuccessfully, serviceRequestHasBeenDeleted } from '../common/common';
...
export class DeleteServiceTypeByUserSendMailService {
    ...
    async deleteSirviceTypeSendEmail(to: string, serviceSubType: string) {
        try {
            ...
            const htmlTemplate = emailDeleteServiceTypeTemplates.ServiceTypeDeleted(serviceSubType);
            const mailOptions = {
                from: USER_EMAIL,
                to,
