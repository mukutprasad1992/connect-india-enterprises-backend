...
export class CreateServiceTypeController {
    ...
    async createServiceType(
        ...
        this.logger.doLog(`${requestReceivedToCreateServiceTypeByUserID} ${userId}`, 'success');
        this.logger.doLog(`Payload: ${JSON.stringify(createServiceTypeDto)}`, 'success');

        try {
            const serviceTypeSchemaResponse = await this.createServiceTypeService.createServiceType(
                userId,
            ...
            if (serviceTypeSchemaResponse.status === true) {
                ...
                return res.status(201).send({
                    status: true,
                    message: serviceTypeSchemaResponse.message || serviceTypeCreatedSuccessfully,
