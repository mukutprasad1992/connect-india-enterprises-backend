...
export class DeleteServiceTypeByIdService {
  ...
  async deleteServiceTypeById(id: number, userId: number): Promise<any> {
    this.logger.doLog(
      `${startDeletingServiceTypeId}=${id} by userId=${userId}`,
      'success',
    );
    ...
    try {
      const userIdQuery = `
                ...
                WHERE sr.id = ?;
            `;
      const getUserIdByServiceTypeId = await this.dataSource.query(
        userIdQuery,
        [id],
      );
      ...
      if (!getUserIdByServiceTypeId || getUserIdByServiceTypeId.length === 0) {
        this.logger.doLog(
          `${serviceTypeNotFoundOrAlreadyDeletedId}=${id}`,
          'fail',
        );
        return {
          status: false,
      ...
      if (!result || result.affectedRows === 0) {
        this.logger.doLog(
          `${failedToDeleteServiceTypeOrAlreadyDeletedId}=${id}`,
          'fail',
        );
        return {
          status: false,
      ...
      if (!notification) {
        this.logger.doLog(
          `${notificationCreationFailedForDeletedServiceTypeId}=${id}`,
          'fail',
        );
        return {
          status: false,
      ...
      this.logger.doLog(
        `${notificationCreatedForDeletedServiceTypeId}=${id}`,
        'success',
      );
      ...
      if (!sendEmailToUser) {
        this.logger.doLog(
          `${failedToSendDeletionEmailTo} ${email} ${forServiceTypeId}=${id}`,
          'fail',
        );
        return {
          status: false,
      ...
      this.logger.doLog(
        `${deletionEmailSentTo} ${email} ${forServiceTypeId}=${id} `,
        'success',
      );

      this.logger.doLog(
        `${serviceTypeDeletionCompletedSuccessfullyId} = ${id} `,
        'success',
      );
      ...
      this.logger.doLog(
        `${errorDeletingServiceTypeId} = ${id}: ${error.message} `,
        'fail',
      );
      return {
        status: false,
