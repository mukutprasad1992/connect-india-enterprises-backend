...
export class UpdateServiceTypeStatusService {
  ...
  private async getTableName(serviceId: number): Promise<string> {
    ...
    this.logger.doLog(
      `${resolvedTable} = ${table} ${forServiceId} = ${serviceId}`,
      'success',
    );
    return table;
  }

  private async getServiceTypeById(
    id: number,
    serviceId: number,
  ): Promise<any | null> {
    ...
    if (!table) {
      this.logger.doLog(
        `${tableNotFoundWhileFetchingServiceTypeId} = ${id} serviceId=${serviceId}`,
        'fail',
      );
      return {
        status: false,
  ...
  async updateServiceTypeStatus(
    ...
    try {
      ...
      if (
        !['Approved', 'Rejected', 'In Progress', 'Pending'].includes(status)
      ) {
        this.logger.doLog(
      ...
      const getStatusMessage = (
        status: string,
        formattedServiceSubType: string,
      ) => {
        switch (status) {
          case 'Approved':
      ...
      if (updateResult) {
        ...
        await this.NotificationMailService.sendNotificationEmail(
          email,
          status,
          serviceSubType,
        );
