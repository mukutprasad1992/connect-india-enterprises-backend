...
export class CreateServiceTypeService {
  ...
  async createServiceType(
    ...
    this.logger.doLog(
      `${startCreatingServiceTypeForUserId}=${userId}`,
      'success',
    );
    ...
    try {
      ...
      if (!serviceSubType) {
        this.logger.doLog(
          `${invalidServiceSubType}: ${dto.serviceSubType}`,
          'fail',
        );
        return {
          status: false,
      ...
      if (!serviceRequestId) {
        this.logger.doLog(
          `${failedToCreateServiceRequestForUserId}=${userId}`,
          'fail',
        );
        return { status: false, message: failedToCreateServiceRequest };
      }
      this.logger.doLog(
        `${serviceRequestCreatedWithId}=${serviceRequestId}`,
        'success',
      );
      ...
      if (!basicDetailsId) {
        this.logger.doLog(
          `${failedToCreateBasicDetailsForUserId}=${userId}`,
          'fail',
        );
        return { status: false, message: failedToCreateBasicDetails };
      }
      this.logger.doLog(
        `${basicDetailsCreatedWithId}=${basicDetailsId}`,
        'success',
      );
      ...
      if (!investmentId) {
        this.logger.doLog(
          `${failedToRetrieveInvestmentIdForServiceRequestId}=${serviceRequestId}`,
          'fail',
        );
        return {
          status: false,
      ...
      this.logger.doLog(
        `${investmentDetailsCreatedWithId}=${investmentId}`,
        'success',
      );
      ...
      if (!notification) {
        this.logger.doLog(
          `${notificationCreationFailedForUserId}=${userId}`,
          'fail',
        );
        return { status: false, message: notificationCreationFailed };
      }
      this.logger.doLog(
        `${notificationCreatedSuccessfullyForUserId}=${userId}`,
        'success',
      );
      ...
      const service =
        dto.activeSteps === 'basicDetails'
          ...
          : 'Investment Details';
      this.logger.doLog(
        `${serviceTypeCreationCompletedSuccessfullyForUserId}=${userId}`,
        'success',
      );
