...
export class UpdateServiceTypeByIdService {
  ...
  ): Promise<ServiceTypeSchema | null> {
    this.logger.doLog(
      `${fetchingServiceRequestId} = ${serviceRequestId}`,
      'success',
    );
  ...
  private async insertAndReturnId(
    ...
    if (result?.insertId) {
      this.logger.doLog(
        `${insertedRecordWithId} = ${result.insertId}`,
        'success',
      );
    }
  ...
  async updateBasicDetails(
    ...
    this.logger.doLog(
      `${updatingBasicDetailsId} = ${basicDetailsId} by userId=${userId}`,
      'success',
    );
    const query = `
  ...
  async savePersonalDetails(
    ...
    this.logger.doLog(
      `${savingPersonalDetailsForUserId} = ${userId}`,
      'success',
    );
    const query = `
    ...
    if (result.insertId && result.insertId !== 0) {
      this.logger.doLog(
        `${insertedPersonalDetailsId} = ${result.insertId}`,
        'success',
      );
  ...
  async saveNomineeDetails(
    ...
    this.logger.doLog(
      `${savingNomineeDetailsForUserId} = ${userId}`,
      'success',
    );
    if (id) {
      const query = `
      ...
      this.logger.doLog(
        `${insertedNomineeDetailsId} = ${result.insertId}`,
        'success',
      );
  ...
  async saveDocuments(
    ...
    if (id) {
      ...
      this.logger.doLog(
        `${insertedDocumentsId} = ${result.insertId}`,
        'success',
      );
      return result.insertId || null;
    }
  ...
  async updateServiceTypeById(
    ...
    try {
      ...
      let finalActiveStep =
        requestStepOrder > currentStepOrder ? activeSteps : currentActiveSteps;
      ...
      // === Review
        ...
        this.logger.doLog(
          `${markingReviewCompleted}=${serviceRequestId} ${andUserId}=${userId}`,
          'success',
        );
      ...
      if (detailId) {
        if (serviceRequestExists) {
          ...
          this.logger.doLog(
            `${updatedInvestmentdetailsForServiceRequestId} = ${serviceRequestId}`,
            'success',
          );
        } else {
          const invQuery = `
      ...
      this.logger.doLog(
        `${errorInUpdateServiceTypeById} ${error.message}`,
        'fail',
      );
      return {
        status: false,
