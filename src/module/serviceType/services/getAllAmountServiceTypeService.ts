...
export class GetTotalAmountServiceTypeService {
    ...
    private async fetchTotalAmountByServiceId(
      ...
      WHERE serviceId = ?
    `;
        this.logger.doLog(`${runningTotalAmountQueryForServiceId} = ${serviceId}`, 'success');
    ...
    async getTotalAmountServiceType(serviceId: number): Promise<any> {
        try {
            this.logger.doLog(`${fetchingTotalAmountForServiceId} = ${serviceId}`, 'success');
            const response = await this.fetchTotalAmountByServiceId(serviceId);

            if (!response || response.totalAmount === null) {
                this.logger.doLog(`${noTotalAmountFoundForServiceId} = ${serviceId}`, 'fail');
                return {
                    status: false,
            ...
            this.logger.doLog(`${errorRetrievingTotalAmount} ${error.message}`, 'fail');
            return {
                status: false,
