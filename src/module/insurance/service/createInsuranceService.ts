...
export class CreateInsuranceService {
    ...
    async createInsurance(userId: number, dto: CreateInsuranceDTO): Promise<any> {
        ...
        try {
            ...
            if (!user) {
                this.logger.doLog(`${createInsuranceServiceUserNotFound} (userId: ${userId})`, 'warn');
            ...
            const insuranceBasicDetailsId = await this.addBasicDetails(userId, dto.aadharNumber, dto.panNumber);
