...
export class UpdateInsuranceDTO {
  ...
  static getValidationSchema() {
    ...
    const personalDetailsSchema = Joi.object({
      ...
      heightCM: Joi.number().required().messages({
      ...
      weightKG: Joi.number().required().messages({
    ...
    return baseSchema
      .when(Joi.object({ activeSteps: Joi.valid('basicDetails') }).unknown(), {
      ...
      .when(
        Joi.object({ activeSteps: Joi.valid('nomineeDetails') }).unknown(),
        {
