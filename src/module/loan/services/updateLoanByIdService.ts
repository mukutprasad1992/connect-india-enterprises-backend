...
export class UpdateLoanByIdService {
  ...
  async getServiceRequestById(
  ...
  ): Promise<LoanSchema | null> {
    this.logger.doLog(
      `${fetchingLoanDetailsForServiceRequestId} ${serviceRequestId}`,
      'info',
    );
    ...
    this.logger.doLog(
      `${serviceRequest} ${serviceRequestId} ${exists ? 'found' : 'not found'}`,
      'info',
    );
    return exists;
  }
  ...
  async updatePersonalDetails(
    ...
    this.logger.doLog(
      `${updatingPersonalDetailsForUserId} ${userId}, basicDetailsId: ${basicDetailsId}`,
      'info',
    );
    const query = `
    ...
    this.logger.doLog(
      `${personalDetailsUpdate} ${success ? 'successful' : 'failed'}`,
  ...
  async saveContactdetails(
    ...
    this.logger.doLog(
      `${savingContactDetailsForUserId} ${userId}, contactId: ${id}`,
      'info',
    );
    const query = `
    ...
    const detailId =
      result.insertId && result.insertId !== 0 ? result.insertId : id;
  ...
  async saveEmploymentDetails(
    ...
    this.logger.doLog(
      `${savingEmploymentDetailsForUserId} ${userId}, employmentId: ${id}`,
      'info',
    );
    ...
    if (id) {
      ...
      const success = result.affectedRows > 0;
      this.logger.doLog(
        `${employmentDetailsUpdate} ${success ? 'successful' : 'failed'}`,
        success ? 'success' : 'warn',
      );
      ...
      this.logger.doLog(
        `${employmentDetailsInsertedWithID} ${insertedId}`,
        'info',
      );
      return insertedId;
    }
  ...
  async saveReferencedetails(
    ...
    this.logger.doLog(
      `${savingReferenceDetailsForUserId} ${userId}, referenceId: ${id}`,
      'info',
    );
    ...
    if (id) {
      ...
      const success = result.affectedRows > 0;
      this.logger.doLog(
        `${referenceDetailsUpdate} ${success ? 'successful' : 'failed'}`,
        success ? 'success' : 'warn',
      );
      return success ? id : null;
    } else {
      ...
      const insertedId = result.insertId || null;
      this.logger.doLog(
        `${referenceDetailsInsertedWithID} ${insertedId}`,
        'info',
      );
      return insertedId;
    }
  ...
  async saveDocuments(
    ...
    this.logger.doLog(
      `${savingDocumentDetailsForUserId} ${userId}, documentsId: ${id}`,
      'info',
    );
    ...
    if (id) {
      ...
      const success = result.affectedRows > 0;
      this.logger.doLog(
        `Documents update ${success ? 'successful' : 'failed'}`,
        success ? 'success' : 'warn',
      );
      return success ? id : null;
    } else {
  ...
  async updateLoanById(
    ...
    this.logger.doLog(
      `${startingLoanUpdateForServiceRequestId} ${serviceRequestId}, userId: ${userId}`,
      'info',
    );
    try {
      let serviceRequestExists =
        await this.getServiceRequestById(serviceRequestId);
      ...
      const currentStepOrder = currentActiveSteps
        ? stepOrder[currentActiveSteps]
        : 0;
      ...
      switch (activeSteps) {
        case 'personalDetails':
          ...
          if (!updatedPersonal) {
            this.logger.doLog(
              `${failedToSavePersonalDetailsForLoanId} ${serviceRequestId}`,
              'warn',
            );
        ...
        case 'contactDetails':
          ...
          if (!detailId) {
            this.logger.doLog(
              `${failedToSaveContactDetailsForLoanId} ${serviceRequestId}`,
              'warn',
            );
        ...
        case 'employmentDetails':
          ...
          if (!detailId) {
            this.logger.doLog(
              `${failedToSaveEmploymentDetailsForLoanId} ${serviceRequestId}`,
              'warn',
            );
        ...
        case 'referenceDetails':
          ...
          if (!detailId) {
            this.logger.doLog(
              `${failedToSaveReferenceDetailsForLoanId} ${serviceRequestId}`,
              'warn',
            );
        ...
        case 'documents':
          ...
          if (!detailId) {
            this.logger.doLog(
              `${failedToSaveDocumentsForLoanId} ${serviceRequestId}`,
              'warn',
            );
        ...
        case 'review':
          ...
          this.logger.doLog(
            `${reviewStepCompletedForLoanId} ${serviceRequestId}`,
            'success',
          );
      ...
      this.logger.doLog(
        `Step ${formattedStep} ${completedSuccessfullyForLoanId} ${serviceRequestId}`,
        'success',
      );
      ...
      this.logger.doLog(
        `${errorUpdatingLoanId} ${serviceRequestId}, error: ${error.message}`,
        'error',
      );
      return {
        status: false,
