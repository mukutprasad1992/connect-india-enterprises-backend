...
export class CreateLoanService {
  ...
  async getServiceSubTypeId(
    ...
    this.logger.doLog(
      `${fetchingServiceSubTypeIDFor} ${serviceSubType}`,
      'info',
    );
  ...
  async createLoan(userId: number, dto: CreateLoanDTO): Promise<any> {
    ...
    try {
      ...
      if (!loanId)
        return {
          status: false,
          message: failedToRetrieveTheIDOfTheLastInsertedLoan,
        };
      ...
      this.logger.doLog(
        `${loanCreatedSuccessfullyWithID} ${loanId} (userId: ${userId})`,
        'success',
      );
      ...
      if (!notification) {
        this.logger.doLog(
          `${notificationCreationFailedForLoanId} ${loanId}`,
          'warn',
        );
        return { status: false, message: notificationCreationFailed };
      }
      this.logger.doLog(
        `${notificationCreatedSuccessfullyForLoanId} ${loanId}`,
        'success',
      );
