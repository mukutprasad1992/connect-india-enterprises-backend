...
export class UpdateServiceSubTypeByIdService {
    ...
    async updateServiceSubType(ledgerId: number, updatedData: any, userId: number): Promise<any> {
        this.logger.doLog(`${updatingServiceSubTypeID} ${ledgerId} ${byUserID} ${userId}`, 'info');
        ...
        try {
            ...
            if (!profile || !profile.data) {
                this.logger.doLog(`${serviceSubTypeNotFoundForUpdateID} ${ledgerId}`, 'warn');
                return {
                    status: false,
                    message: serviceSubTypeNotFound,
                    data: null
            ...
            this.logger.doLog(`${serviceSubTypeUpdateQueryExecutedSuccessfullyForID} ${ledgerId}`, 'info');

            const updatedProfile = await this.updateServiceSubTypeById(ledgerId);

            this.logger.doLog(`${serviceSubTypeUpdatedSuccessfullyForID} ${ledgerId}`, 'success');
            return {
                status: true,
            ...
            this.logger.doLog(`${errorUpdatingServiceSubTypeID} ${ledgerId}, error: ${error.message}`, 'error');
            return {
                status: false,
