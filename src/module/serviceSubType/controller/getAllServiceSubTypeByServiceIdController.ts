...
import {
  ...
  byUserID,
  errorFetchingServiceSubTypesForServiceID,
  fetchedSuccessfullyByUserID,
  noServiceSubTypesFoundForServiceID,
  requestedByUserID,
  requestedServiceSubTypesForServiceID,
  serviceSubTypesForServiceID,
...
export class GetServiceSubTypeByServiceIdController {
  ...
  async getProfile(
    ...
    this.logger.doLog(
      `User ID: ${userId} ${requestedServiceSubTypesForServiceID} ${serviceId}`,
      'info',
    );

    try {
      const serviceSubTypeResponse =
        await this.getServiceSubTypeByServiceIdService.getServiceSubTypeByServiceId(
          serviceId,
        );
