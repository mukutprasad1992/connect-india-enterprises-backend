...
export class DeleteProfileImageService {
  ...
  private async deleteFileFromS3(key: string) {
    ...
    if (!bucket || !key) {
      this.logger.doLog(
        `${invalidBucketOrKeyWhileDeletingFromS3Bucket} = ${bucket}, key=${key}`,
        'warn',
      );
      return;
    }
    ...
    try {
      ...
      for (const version of allVersions) {
        if (version.Key === key && version.VersionId) {
          ...
          this.logger.doLog(
            `${deletedVersion} ${version.VersionId} of key: ${key}`,
            'success',
          );
        }
      }
      ...
      this.logger.doLog(
        `${successfullyDeletedAllVersionsOfKey} ${key}`,
        'success',
      );
    } catch (error: any) {
      this.logger.doLog(
  ...
  async deleteProfileImage(userId: number) {
    this.logger.doLog(
      `${attemptingToDeleteProfileImageForUserId} ${userId}`,
      'info',
    );
    ...
    this.logger.doLog(
      `${deletingProfileImageFromS3ForUserId} ${userId}, key: ${user.profileImageKey}`,
      'info',
    );
    await this.deleteFileFromS3(user.profileImageKey);

    this.logger.doLog(
      `${clearingProfileImageFieldsFromDatabaseForUserId} ${userId}`,
      'info',
    );
    ...
    this.logger.doLog(
      `${profileImageDeletedSuccessfullyForUserId} ${userId}`,
      'success',
    );
    return {
      status: true,
