    ...
    async updateUser(id: number, updateData: UpdateUserDTO, userId: number): Promise<any> {
        ...
        try {
            ...
            if (updateFields.isEmpty) {
                this.logger.doLog(`${noFieldsToUpdateOrUserNotFoundForId} ${id}`, 'warn');
                return {
                    status: false,
            ...
            if (updateResult.changedRows === 0) {
                this.logger.doLog(`${userNotFoundOrNoChangesAppliedForId} ${id}`, 'warn');
                return {
                    status: false,
                    message: userNotFoundOrNoFieldsWereUpdated,
                    data: null
            ...
            this.logger.doLog(`${errorOccurredWhileUpdatingUserId} ${id}, error: ${error.message}`, 'error');
            return {
                status: false,
