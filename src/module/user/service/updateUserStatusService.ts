  ...
  async updateUserStatus(
    ...
    try {
      ...
      if (status !== 'Enable' && status !== 'Disable') {
        this.logger.doLog(
          `${invalidStatusValueProvidedForUserId} ${id}`,
          'warn',
        );
        return {
          status: false,
      ...
      if (updateResult.affectedRows === 0) {
        this.logger.doLog(
          `${noChangesMadeWhileUpdatingStatusForUserId} ${id}`,
          'warn',
        );
        return {
          status: false,
      ...
      this.logger.doLog(
        `${userStatusUpdatedSuccessfullyForId} ${id} to ${status}`,
        'success',
      );

      const mail =
        await this.vendorBlockOrUnblockMailService.emailVendorBlockOrUnblock(
          updatedUser.email,
          status,
      ...
      this.logger.doLog(
        `${errorOccurredWhileUpdatingStatusForUserId} ${id}, error: ${error.message}`,
        'error',
      );
      return {
        status: false,
