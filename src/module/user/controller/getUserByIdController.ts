...
import {
  errorOccurredWhileRetrievingUserID,
  failedToRetrieveUserForID,
  getUserByIDRequestReceivedForID,
  invalidUserIDProvided,
  userRetrievedSuccessfullyForID,
...
export class GetUserByIdController {
  ...
  async getUserById(@Param('id') id: number, @Res() res) {
    ...
    try {
      ...
      this.logger.doLog(
        `${errorOccurredWhileRetrievingUserID} ${id}, Error: ${error.message}`,
        'error',
      );
