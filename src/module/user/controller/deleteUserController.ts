    ...
    async delete(@Param('id') id: number, @Res() res) {
        ...
        try {
            ...
            if (userResponse.status === true) {
                ...
                this.logger.doLog(`${failedToDeleteUserID} ${id}, Message: ${userResponse.message}`, 'warn');
                return res.status(400).json({
                    status: userResponse.status,
                    message: userResponse.message,
            ...
            this.logger.doLog(`${errorOccurredWhileDeletingUserID} ${id}, Error: ${error.message}`, 'error');
