...
export class UpdateUserStatusByIdController {
    ...
    async updateVoucherById(
        ...
        this.logger.doLog(`${receivedRequestToUpdateUserStatusForID} ${id}`, 'info');

        try {
            const userId = req.user.id;
            const { status } = updateUserDTO;
            this.logger.doLog(`${requestedByUserID} ${userId} — Target ID: ${id} — New Status: ${status}`, 'warn');
            ...
            if (voucherResponse.status) {
                this.logger.doLog(`${userStatusUpdatedSuccessfullyForID} ${id}`, 'success');
                ...
                this.logger.doLog(`${failedToUpdateUserStatusForID} ${id} — ${voucherResponse.message}`, 'warn');
            ...
            this.logger.doLog(`${errorWhileUpdatingUserStatusForID} ${id} — ${error.message}`, 'error');
