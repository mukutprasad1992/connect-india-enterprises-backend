...
export class GetAllServicesController {
    constructor(private readonly getALLServiceByIdmentService: GetALLServiceByIdmentService) { }
    @UseGuards(AuthGuard)
    @Get()
    async getUserServices(@Res() res, @Req() req) {
        try {
            const userId = req.user.id;
            const servicesResponse = await this.getALLServiceByIdmentService.getAllServicesByUser();
