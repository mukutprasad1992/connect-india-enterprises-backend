...
export class UpdateServiceByIdController {
    constructor(private readonly updateServiceByIdService: UpdateServiceByIdService) { }

    @UseGuards(AuthGuard)
    @Put()
    async updateServiceById(
        @Param('id') id: number,
        @Body(new ValidationService(UpdateServiceDTO.serviceSchema)) updateServiceDTO: UpdateServiceDTO,
        @Res() res,
        @Req() req
    ) {
        try {
            const userId = req.user.id;
            const updateResponse = await this.updateServiceByIdService.updateServiceById(id, userId, updateServiceDTO);
