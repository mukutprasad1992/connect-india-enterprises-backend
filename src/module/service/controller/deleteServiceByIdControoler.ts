...
export class DeleteServiceByIdController {
  constructor(
    private readonly deleteServiceByIdService: DeleteServiceByIdService,
  ) {}

  @UseGuards(AuthGuard)
  @Delete()
  async deleteServiceById(@Param('id') id: number, @Res() res, @Req() req) {
    try {
      const userId = req.user.id;
      const deleteResponse =
        await this.deleteServiceByIdService.deleteServiceById(id);
