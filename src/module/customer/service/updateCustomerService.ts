...
export class UpdateCustomerService {
  ...
  async getCustomerById(id: number): Promise<CustomerSchema | null> {
    ...
    if (customer.length > 0) {
      this.logger.doLog(
        `${customerRetrievedSuccessfully} (ID: ${id})`,
        'success',
      );
      return customer[0];
    } else {
  ...
  async updateCustomer(
    ...
    this.logger.doLog(
      `${attemptingToUpdateCustomerId} ${id} ${byUserId} ${userId})`,
      'info',
    );
    ...
    try {
      ...
      this.logger.doLog(
        `${customerUpdatedSuccessfullyInDB} (ID: ${id})`,
        'success',
      );
      ...
      this.logger.doLog(
        `${creatingNotificationForUpdatedCustomer} (ID: ${id})`,
        'info',
      );
      const notification =
      ...
      if (!notification) {
        this.logger.doLog(
          `${notificationCreationFailedForCustomer} (ID: ${id})`,
          'error',
        );
        return {
          status: false,
      ...
      this.logger.doLog(
        `${customerUpdateNotificationCompletedSuccessfully} (ID: ${id})`,
        'success',
      );
      return {
        status: true,
