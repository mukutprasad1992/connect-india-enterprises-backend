...
export class GetAllCustomerService {
    ...
    async getAllCustomer(userId: number): Promise<any> {
        ...
        try {
            ...
            const { roleId } = user;
            this.logger.doLog(`${userFoundUserId} ${userId}, roleId: ${roleId}`, 'info');
            ...
            this.logger.doLog(`${customersRetrievedSuccessfullyForUserId} ${userId}, count: ${customers.length}`, 'success');
            return {
                status: true,
            ...
            this.logger.doLog(`${errorFetchingCustomersForUserId} ${userId} - ${error.message}`, 'error');
            return {
                status: false,
    ...
    private async getUserById(id: number): Promise<any> {
        ...
        try {
            ...
            this.logger.doLog(`${errorFetchingUserWithID} ${id}: ${error.message}`, 'error');
            return null;
        }
    }
}
