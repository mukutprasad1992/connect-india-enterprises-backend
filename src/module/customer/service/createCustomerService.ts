...
export class CreateCustomerService {
  ...
  async getCustomerById(id: number): Promise<CustomerSchema | null> {
    ...
    this.logger.doLog(
      `${customerFetched} ${customer.length > 0 ? 'found' : 'not found'}`,
      customer.length > 0 ? 'success' : 'warn',
    );
  ...
  async createCustomer(
    ...
    if (existingCustomer) {
      this.logger.doLog(
        `${customerWithEmail} ${email} ${alreadyExists}`,
        'warn',
      );
      return {
        status: false,
    ...
    try {
      ...
      if (response) {
        const lastInserted = await this.dataSource.query(
        ...
        if (!notification) {
          this.logger.doLog(
            `${notificationCreationFailedForCustomerID} ${lastInsertedId}`,
            'warn',
          );
          return {
            status: false,
        ...
        this.logger.doLog(
          `${customerCreateSuccessfullyWithID} ${lastInsertedId}`,
          'success',
        );
        return {
          status: true,
