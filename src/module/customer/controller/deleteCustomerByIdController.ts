...
import {
  byUserId,
  deleteCustomerByIdControllerCustomer,
  deleteCustomerByIdControllerFailedToDeleteCustomer,
  deleteCustomerByIdControllerRequestReceivedToDeleteCustomer,
  deleteCustomerByIdControllerUnexpectedErrorWhileDeletingCustomer,
  deletedSuccessfullyByUser,
...
export class DeleteCustomerByIdController {
  ...
  @UseGuards(AuthGuard)
  @Delete()
  async deleteCustomerByVendorId(
    @Param('id') id: number,
    @Res() res,
    @Req() req,
  ) {
    ...
    try {
      const customersResponse =
        await this.deleteCustomerByIdService.deleteCustomerById(id);
