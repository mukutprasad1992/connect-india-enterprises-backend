...
export class CreateNotificationService {
  constructor(
    ...
    private notificationRepo: Repository<NotificationSchema>,
  ...
  async createNotification(dto: CreateNotificationDTO): Promise<any> {
    this.logger.doLog(
      `${creatingNotificationForUserRoleId} = ${dto.userRoleId}, userId=${dto.userId}`,
      'info',
    );
    ...
    try {
      ...
      this.logger.doLog(
        `${notificationInsertedWithId} = ${notificationId}`,
        'success',
      );
      ...
      if (notification.length > 0) {
        this.logger.doLog(
          `${notificationFetchedSuccessfullyForId} = ${notificationId}`,
          'success',
        );
        return {
          status: true,
        ...
        this.logger.doLog(
          `${failedToFetchNotificationAfterCreationId} = ${notificationId}`,
          'warn',
        );
        return {
          status: false,
      ...
      this.logger.doLog(
        `${notificationCreationFailed}: ${error.message}`,
        'error',
      );
      return {
        status: false,
