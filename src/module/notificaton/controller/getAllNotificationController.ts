...
import { errorFetchingNotifications, failedToFetchNotifications, fetchedAllNotificationsSuccessfullyTotal, receivedRequestToFetchAllNotifications, somethingWentWrong } from '../common/notificationMessage';
    ...
    async getAllNotifications(@Res() res, @Req() req) {
        ...
        try {
            ...
            if (response.status === true) {
                this.logger.doLog(`${fetchedAllNotificationsSuccessfullyTotal} ${response.data?.length || 0}`, 'success');
                ...
                this.logger.doLog(`${failedToFetchNotifications} ${response.message}`, 'warn');
