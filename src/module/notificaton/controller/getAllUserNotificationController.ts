...
import {
  ...
  failedToFetchNotificationsForUserId,
  notificationsForUserId,
  receivedRequestToFetchNotificationsForUserId,
  somethingWentWrong,
...
export class GetAllUserNotificationController {
  ...
  async getAllUserNotifications(@Res() res, @Req() req) {
    ...
    this.logger.doLog(
      `${receivedRequestToFetchNotificationsForUserId} ${userId}`,
      'info',
    );

    try {
      const response =
        await this.getAllUserNotificationService.getAllNotifications(userId);

      if (response.status === true) {
        this.logger.doLog(
          `Fetched ${response.data?.length || 0} ${notificationsForUserId} ${userId}`,
          'success',
        );
        ...
        this.logger.doLog(
          `${failedToFetchNotificationsForUserId} ${userId}, Reason: ${response.message}`,
          'warn',
        );
      ...
      this.logger.doLog(
        `${errorFetchingNotificationsForUserId} ${userId}, Error: ${error.message}`,
        'error',
      );
