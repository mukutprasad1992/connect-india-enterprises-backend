...
import {
  createNotificationRequestReceived,
  errorCreatingNotification,
  failedToCreateNotificationReason,
...
export class CreateNotificationController {
  ...
  @Post()
  async create(
    @Body(new JoiValidationNotification())
    ...
    try {
      const response = await this.createNotificationService.createNotification(
      ...
      if (response.status === true) {
        ...
        // Custom log: failure
        this.logger.doLog(
          `${failedToCreateNotificationReason} ${response.message}`,
          'warn',
        );
      ...
      // Custom log: error
      this.logger.doLog(
        `${errorCreatingNotification}, Error: ${error.message}`,
        'error',
      );
