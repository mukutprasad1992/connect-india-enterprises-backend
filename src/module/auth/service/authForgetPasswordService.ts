...
export class ForgetPasswordService {
  ...
  async handleForgotPassword(email: string): Promise<any> {
    this.logger.doLog(
      `${passwordResetRequestReceivedEmail} ${email}`,
      'success',
    );
    ...
    try {
      ...
      this.logger.doLog(
        `${resetTokenGeneratedUserId} ${user.data.id}`,
        'success',
      );
      ...
      if (!saveToken) {
        this.logger.doLog(
          `${failedToSaverResetTokenUserId} ${user.data.id}`,
          'fail',
        );
        return {
          status: false,
      ...
      this.logger.doLog(
        `${resetTokenSavedSuccessfullyUserId} ${user.data.id}`,
        'success',
      );
      ...
      if (mail.status === true) {
        this.logger.doLog(
          `${passwordResetEmailSentSuccessfullyEmail} ${email}`,
          'success',
        );
        return {
          status: true,
      ...
      this.logger.doLog(
        `${failedToSendPasswordResetEmailEmail} ${email}`,
        'fail',
      );
      return {
        status: false,
      ...
      this.logger.doLog(
        `${errorDuringPasswordResetEmail} ${email}. Error: ${error.message}`,
        'fail',
      );
      return {
        status: false,
  ...
  private async saveResetToken(id: string, resetToken: string): Promise<any> {
    try {
      ...
      this.logger.doLog(
        `${errorSavingResetTokenUserId} ${id}. Error: ${error.message}`,
        'fail',
      );
      return {
        status: false,
