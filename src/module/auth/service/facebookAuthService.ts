...
export class FacebookAuthService {
    ...
    async login(oauthUser: any): Promise<any> {
        ...
        this.logger.doLog(`${facebookLoginRequestReceivedEmail} ${email}, providerId: ${providerId}`, 'success');
        ...
        try {
            if (email) {
                ...
                this.logger.doLog(`${userLookupByEmailEmail} ${email}, found: ${result.length}`, 'success');
            }
            ...
            if (result.length === 0) {
                this.logger.doLog(`${noExistingUserFoundCreatingNewUserEmail} ${email}`, 'success');
                const insertQuery = `
            ...
            const accessToken = jwt.sign({ id: user.id }, process.env.JWT_SECRET || 'default_secret', { expiresIn: '1h' });
            ...
            this.logger.doLog(`${errorInFacebookLoginEmail} ${email}, providerId: ${providerId}, Error: ${error.message}`, 'fail');
            return {
                status: false,
