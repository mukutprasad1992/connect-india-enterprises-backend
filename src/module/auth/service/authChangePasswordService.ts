...
export class ChangePasswordService {
    ...
    async validateUser(userId: string, oldPassword: string): Promise<UserSchema | null> {
        this.logger.doLog(`${validatingUserPorPasswordChangeUserId} ${userId}`, 'success');
        ...
        this.logger.doLog(`${userValidatedSuccessfullyUserId} ${userId}`, 'success');
    ...
    async changePassword(id: string, changePasswordDTO: ChangePasswordDTO): Promise<any> {
        this.logger.doLog(`${passwordChangeRequestReceivedUserId} ${id}`, 'success');

        try {
            const user = await this.validateUser(id, changePasswordDTO.oldPassword);

            if (!user) {
                this.logger.doLog(`${passwordChangeFailedIncorrectOldPasswordUserId} ${id}`, 'fail');
                return {
                    status: false,
            ...
            const response = await this.dataSource.query(updateQuery, [hashedNewPassword, id]);
            ...
            this.logger.doLog(`${passwordChangedSuccessfullyUserId} ${id}`, 'success');
            ...
            this.logger.doLog(`${errorDuringPasswordChangeUserId} ${id}. Error: ${error.message}`, 'fail');
