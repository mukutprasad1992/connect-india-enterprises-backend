...
export class ResetPasswordService {
  ...
  async resetPassword(resetToken: string, newPassword: string): Promise<any> {
    this.logger.doLog(
      `${resetPasswordRequestReceivedResetToken} ${resetToken}`,
      'success',
    );
    ...
    try {
      ...
      this.logger.doLog(
        `${resetTokenVerifiedUserId} ${decodedToken.id}`,
        'success',
      );
    } catch (error: any) {
      this.logger.doLog(
        `${invalidOrExpiredResetTokenError} ${error.message}`,
        'fail',
      );
      return {
        status: false,
    ...
    try {
      ...
      this.logger.doLog(
        `${errorupdatingPasswordUserId} ${id}. Error: ${error.message}`,
        'fail',
      );
      return {
        status: false,
  ...
  private async isUserExist(id: string): Promise<any> {
    try {
      ...
      this.logger.doLog(
        `${errorCheckingUserExistenceUserId} ${id}. Error: ${error.message}`,
        'fail',
      );
      return {
        status: false,
  ...
  private async isResetPasswordExist(resetToken: string): Promise<any> {
    try {
      ...
      if (result.length === 0) {
        this.logger.doLog(
          `${resetTokenExistenceCheckNotFoundToken} ${resetToken}`,
          'fail',
        );
        return null;
      }
      ...
      this.logger.doLog(
        `${resetTokenExistenceCheckFoundUserId} ${result[0].id}`,
        'success',
      );
      return result[0];
    } catch (error: any) {
      this.logger.doLog(
        `${errorCheckingResetTokenError} ${error.message}`,
        'fail',
      );
      return {
        status: false,
  ...
  private async updateUserPassword(
    ...
    try {
      ...
      if (result.affectedRows === 0) {
        this.logger.doLog(
          `${noUserUpdatedOrNoChangesMadeUserId} ${id}`,
          'fail',
        );
        return {
          status: false,
  ...
  private async clearResetToken(id: string): Promise<any> {
    try {
      ...
      this.logger.doLog(
        `${errorClearingResetTokenUserId} ${id}. Error: ${error.message}`,
        'fail',
      );
      return {
        status: false,
