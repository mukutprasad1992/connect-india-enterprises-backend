...
export class LoginController {
    ...
    async login(@Body(new JoiValidationAuth(LoginDTO.loginSchema)) loginDto: LoginDTO, @Res() res) {
        ...
        try {
            ...
            if (result.status === false) {
                this.logger.doLog(`${loginFailedEmail} ${loginDto.email}, Reason: ${result.message}`, 'fail');
            ...
            this.logger.doLog(`${errorDuringLoginEmail} ${loginDto.email}, Error: ${error.message}`, 'fail');
    ...
    async getMe(@Req() req, @Res() res) {
        this.logger.doLog(`${fetchLoggedInUserDataUserId} ${req.user.id}`, 'success');
        ...
        try {
            ...
            this.logger.doLog(`${userDataFetchedSuccessfullyUserId} ${req.user.id} `, 'success');
            return res.status(200).json({ status: true, data: user });
        } catch (error) {
            this.logger.doLog(`${errorFetchingUserDataUserId} ${req.user.id}, Error: ${error.message} `, 'fail');
