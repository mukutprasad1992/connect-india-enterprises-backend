...
export class ResetPasswordController {
    ...
    async resetPassword(
        ...
        try {
            const result = await this.ResetPasswordService.resetPassword(token, resetPasswordDTO.newPassword);

            if (result.status === true) {
                this.logger.doLog(`${passwordResetSuccessfulToken} ${token}`, 'success');
                return res.status(200).send({
                    status: result.status,
                    message: result.message,
                });
            } else {
                this.logger.doLog(`${passwordResetFailedToken} ${token}, Reason: ${result.message}`, 'fail');
            ...
            this.logger.doLog(`${errorDuringPasswordResetToken} ${token}, Error: ${error.message}`, 'fail');
