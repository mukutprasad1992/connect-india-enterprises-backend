...
export class GoogleAuthController {
    ...
    async googleCallback(@Req() req, @Res() res) {
        const user = req.user;
        this.logger.doLog(`${googleLoginCallbackReceivedUser} ${user?.email || 'Unknown'}`, 'success');
        ...
        try {
            ...
            if (!result || !result.data || !result.data.accessToken) {
                this.logger.doLog(`${googleLoginFailedInvalidLoginResultForUser} ${user.email}`, 'fail');
            ...
            this.logger.doLog(`${googleLoginSuccessfulUser} ${user.email}`, 'success');
