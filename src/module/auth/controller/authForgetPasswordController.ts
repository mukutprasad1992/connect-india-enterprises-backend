...
export class ForgetPasswordController {
  ...
  async forgotPassword(
    @Body(new JoiValidationAuth(ForgetPasswordDTO.forgetPasswordSchema))
    forgetPasswordDTO: ForgetPasswordDTO,
    @Res() res,
  ) {
    this.logger.doLog(
      `${requestReceivedForgotPasswordForEmail} ${forgetPasswordDTO.email}`,
      'success',
    );

    try {
      const response = await this.forgetPasswordService.handleForgotPassword(
        forgetPasswordDTO.email,
      );

      if (response.status === true) {
        this.logger.doLog(
          `${resetEmailSentSuccessfullyEmail} ${forgetPasswordDTO.email}`,
          'success',
        );
        ...
        this.logger.doLog(
          `${failedToSendResetEmailEmail} ${forgetPasswordDTO.email}, Reason: ${response.message}`,
          'fail',
        );
      ...
      this.logger.doLog(
        `${errorOccurredWhileSendingResetEmailEmail} ${forgetPasswordDTO.email}, Error: ${error.message}`,
        'fail',
      );
