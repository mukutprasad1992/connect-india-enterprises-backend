  ...
  transform(value: any, metadata: ArgumentMetadata) {
    if (
      ...
      throw new BadRequestException({
        status: false,
        errors: inputCannotBeUndefinedOrNull,
      });
    }
        ...
        .map(({ message, context }) => {
          ...
          if (fieldKey && fieldMap[fieldKey]) {
            if (message.includes(isRequired))
              return fieldMap[fieldKey].required;
            if (message.includes(mustBeAValidEmail))
              return fieldMap[fieldKey].email;
            ...
            if (message.includes(failsToMatchTheRequiredPattern))
              return fieldMap[fieldKey].pattern;
          }
