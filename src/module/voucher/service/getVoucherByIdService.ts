...
import { errorRetrievingVoucherForVoucherId, errorRetrievingVouchers, getVoucherByIdServiceCalledForVoucherId, voucherNotFoundForVoucherId, voucherRecordNotFound, voucherRetrievedSuccessfullyForVoucherId, vouchersRetrievedSuccessfully } from '../common/voucherMessage';
...
export class GetVoucherByIdService {
    ...
    async getVoucherById(voucherId: number): Promise<any> {
        this.logger.doLog(`${getVoucherByIdServiceCalledForVoucherId} ${voucherId}`, 'info');
        ...
        try {
            ...
            if (voucher.length === 0) {
                this.logger.doLog(`${voucherNotFoundForVoucherId} ${voucherId}`, 'warn');
                return {
                    status: false,
            ...
            this.logger.doLog(`${voucherRetrievedSuccessfullyForVoucherId} ${voucherId}`, 'success');
            ...
            this.logger.doLog(`${errorRetrievingVoucherForVoucherId} ${voucherId}, error: ${error.message}`, 'error');
            return {
                status: false,
