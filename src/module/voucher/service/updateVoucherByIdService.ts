...
export class UpdateVoucherService {
  ...
  async updateVoucher(
    ...
    this.logger.doLog(
      `${updateVoucherServiceCalledForVoucherId} ${id} by userId: ${userId}`,
      'info',
    );
    ...
    try {
      ...
      if (result.affectedRows > 0) {
        ...
        if (updatedVoucher.length > 0) {
          this.logger.doLog(
            `${voucherUpdatedSuccessfullyForVoucherId} ${id} by userId: ${userId}`,
            'success',
          );
          return {
            status: true,
          ...
          this.logger.doLog(
            `${voucherUpdateFailedNotFoundAfterUpdateForVoucherId} ${id}`,
            'warn',
          );
          return {
            status: false,
        ...
        this.logger.doLog(
          `${voucherUpdateFailedNoRowsAffectedForVoucherId} ${id}`,
          'warn',
        );
        return {
          status: false,
      ...
      this.logger.doLog(
        `${errorUpdatingVoucherId} ${id} by userId: ${userId}, error: ${error.message}`,
        'error',
      );
      return {
        status: false,
