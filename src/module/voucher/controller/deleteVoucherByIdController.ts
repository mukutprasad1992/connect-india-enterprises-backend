...
import {
  deleteVoucherByIdControllerInvokedForVoucherID,
  errorDeletingVoucherID,
  failedToDeleteVoucherID,
  voucherDeletedSuccessfullyID,
...
export class DeleteVoucherByIdController {
  ...
  async deleteVoucherById(@Param('id') id: number, @Res() res) {
    this.logger.doLog(
      `${deleteVoucherByIdControllerInvokedForVoucherID} ${id}`,
      'info',
    );

    try {
      const deleteResponse =
        await this.deleteVoucherByIdService.deleteVoucherById(id);
      ...
      this.logger.doLog(
        `${errorDeletingVoucherID} ${id}, Error: ${error.message}`,
        'error',
      );
