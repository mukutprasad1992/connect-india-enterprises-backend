...
import {
  errorFetchingVoucherID,
  failedToFetchVoucherID,
  getVoucherByIdControllerCalledFetchingVoucherWithID,
  voucherFetchedSuccessfullyID,
...
export class GetVoucherByIdController {
  ...
  async getVocherById(@Param('id') id: number, @Res() res) {
    this.logger.doLog(
      `${getVoucherByIdControllerCalledFetchingVoucherWithID} ${id}`,
      'info',
    );

    try {
      const voucherResponse =
        await this.getVoucherByIdService.getVoucherById(id);
      ...
      this.logger.doLog(
        `${errorFetchingVoucherID} ${id}: ${error.message}`,
        'error',
      );
