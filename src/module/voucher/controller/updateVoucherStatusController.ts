...
import { errorWhileUpdatingVoucherStatusForID, failedToUpdateStatusForVoucherID, forVoucherID, requestedToUpdateStatusTo, statusUpdatedSuccessfullyByUser, updateVoucherStatusByIdControllerCalledByUserID, voucherID } from '../common/voucherMessage';
...
export class UpdateVoucherStatusByIdController {
    ...
    async updateVoucherById(
        ...
        this.logger.doLog(`${updateVoucherStatusByIdControllerCalledByUserID} ${userId} ${forVoucherID} ${id}`, 'info');

        try {
            const { status } = updateVoucherDTO;
            this.logger.doLog(`${requestedToUpdateStatusTo} "${status}"  ${forVoucherID} ${id}`, 'info');
            ...
            if (voucherResponse.status) {
                this.logger.doLog(`${voucherID} ${id} ${statusUpdatedSuccessfullyByUser} ${userId}`, 'success');
            ...
            this.logger.doLog(`${errorWhileUpdatingVoucherStatusForID} ${id}: ${error.message}`, 'error');
