...
import {
  errorWhileUpdatingVoucherID,
  failedToUpdateVoucherID,
  updatedSuccessfullyByUser,
  updateVoucherByIdControllerCalledByUserID,
  voucherID,
...
export class UpdateVoucherByIdController {
  ...
  async updateVoucherById(
    ...
    this.logger.doLog(
      `${updateVoucherByIdControllerCalledByUserID} ${userId} for voucher ID: ${id}`,
      'info',
    );

    try {
      const voucherResponse = await this.updateVoucherService.updateVoucher(
        userId,
        id,
        updateVoucherDTO,
      );
      ...
      if (voucherResponse.status) {
        this.logger.doLog(
          `${voucherID} ${id} ${updatedSuccessfullyByUser} ${userId}`,
          'success',
        );
      ...
      this.logger.doLog(
        `${errorWhileUpdatingVoucherID} ${id}: ${error.message}`,
        'error',
      );
