...
import { errorRetrievingVouchersForVendorID, failedToRetrieveVouchersForVendorID, getAllVoucherByVendorIdControllerCalledForVendorID, internalServerError, vouchersRetrievedSuccessfullyForVendorID } from '../common/voucherMessage';
...
export class GetAllVoucherByVendorIdController {
    ...
    async getAllVouchersByVendor(@Res() res, @Req() req) {
        ...
        this.logger.doLog(`${getAllVoucherByVendorIdControllerCalledForVendorID} ${vendorId}`, 'info');
        ...
        try {
            ...
            if (voucherResponse.status) {
                this.logger.doLog(`${vouchersRetrievedSuccessfullyForVendorID} ${vendorId}`, 'success');
